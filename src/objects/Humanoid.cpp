...
std::string Humanoid::get_profile(){
    ...
    response += "Damage:         " 
        + std::to_string( get_param_incl_bonus(Param_Type::min_damage) + get_ability_modifier(get_param_incl_bonus(Param_Type::str) )) +
        "/" + std::to_string( get_param_incl_bonus(Param_Type::max_damage) + get_ability_modifier(get_param_incl_bonus(Param_Type::str) ));
    return response;
}

int Humanoid::get_ac() { 
    short dex_modifier = get_ability_modifier(get_param_incl_bonus(Param_Type::dex));
    short base_ac = get_param_incl_bonus(Param_Type::ac);
    return base_ac + dex_modifier; // + worn armour
} 

int Humanoid::get_hr() {
    return random(20) + get_ability_modifier(get_param_incl_bonus(Param_Type::str)) ;
}

int Humanoid::get_dr() {
    // no weapon equipped
    short min_damage = get_param_incl_bonus(Param_Type::min_damage);
    short max_damage = get_param_incl_bonus(Param_Type::max_damage);
    short str_modifier = get_ability_modifier(get_param_incl_bonus(Param_Type::str));
